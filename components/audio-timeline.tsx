...
export function useAudioTimeline(backing:string|null,vocal:string|null,backingMuted:boolean,vocalMuted:boolean){
 ...
 return {clips,setClips,lanes,setLanes,add,audible,playback,end:clips.reduce((m,c)=>Math.max(m,c.start+c.duration),0)};
}
...
export function AudioPlayback({clips,time,playing,muted,onTime,onEnded,onError}:{clips:(AudioClip&{muted:boolean})[];time:number;playing:boolean;muted:boolean;onTime:(s:number)=>void;onEnded:()=>void;onError:(message:string)=>void}){
 ...
 useEffect(()=>{playingRef.current=playing;let frame=0,last=performance.now();function tick(now:number){if(playing)clock.current+=(now-last)/1000;last=now;const t=clock.current;for(const c of clips){const a=elements.current.get(c.id);if(!a)continue;const active=playing&&t>=c.start&&t<c.start+c.duration;a.muted=muted||c.muted;if(active){const local=t-c.start;if(Math.abs(a.currentTime-local)>.15)a.currentTime=local;if(a.paused)void a.play().catch(e=>onError(e.message||'Audio playback failed.'))}else{a.pause();if(t>=c.start&&t<c.start+c.duration&&Math.abs(a.currentTime-(t-c.start))>.15)a.currentTime=t-c.start}}if(playing){onTime(t);if(t>=end){onEnded();return}}frame=requestAnimationFrame(tick)}frame=requestAnimationFrame(tick);return()=>{cancelAnimationFrame(frame);elements.current.forEach(a=>a.pause())}},[clips,playing,muted,end]);
