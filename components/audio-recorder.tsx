...
export function AudioRecorder({disabled,onUse}:{disabled:boolean;onUse:(blob:Blob)=>void}){
 ...
 async function start(){if(disabled||pending||recording)return;setBlob(null);setError('');setPending(true);try{
 ...
 return <section className="inspector-section recording-controls"><span>Record audio</span><label>Microphone<select aria-label="Microphone" value={device} disabled={recording||pending||disabled} onChange={e=>setDevice(e.target.value)}><option value="default">System default</option>{devices.filter(d=>d.deviceId&&d.deviceId!=='default').map((d,i)=><option key={d.deviceId} value={d.deviceId}>{d.label||`Microphone ${i+1}`}</option>)}</select></label><button type="button" disabled={disabled||pending} onClick={()=>recording?recorder.current?.stop():void start()} aria-label={recording?'Stop recording':'Record audio'}>{recording?<Square/>:<Mic/>}{recording?'Stop recording':pending?'Connecting…':'Record audio'}</button>{recording&&<p role="status">Recording…</p>}{url&&<><audio controls src={url} aria-label="Recording preview"/><p>Recording stays on this device.</p><div><button type="button" disabled={disabled} onClick={()=>{if(blob)onUse(blob);setBlob(null)}}><Plus/>Add to timeline</button><button type="button" disabled={disabled||pending} onClick={()=>void start()}><RotateCcw/>Record again</button></div></>}{error&&<p role="alert">{error}</p>}</section>;
}
